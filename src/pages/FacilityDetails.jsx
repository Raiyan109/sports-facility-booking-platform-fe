...
import { useGetFacilitiesQuery, useGetSingleFacilityQuery } from "../redux/features/facility/facilityApi";
...
const FacilityDetails = () => {
    ...
    const { data: facilities, error, isLoading: isFacilitiesLoading } = useGetFacilitiesQuery()
    ...
    return (
        <div className="">
            <div className="max-w-[1480px]  container overflow-hidden px-10">
                <Navbar />
                <div className="min-h-screen py-12 space-y-24">
                    ...
                    {/* Other facilities */}
                    <div className="space-y-10">
                        <h1 className="text-3xl text-grayText">Looking for more facilities?</h1>
                        <div className='grid grid-cols-1 lg:grid-cols-3 xl:grid-cols-4 md:grid-cols-2 gap-5'>
                            {isFacilitiesLoading
                                ...
                                : facilities?.data
                                    ?.filter((otherFacility) => otherFacility?.isDeleted === false)
                                    ?.filter((otherFacility) => otherFacility?._id !== facility?.data?._id) // Exclude the current facility
                                    ?.map((otherFacility) => (
                                        <Facility key={otherFacility._id} facility={otherFacility} />
                                    ))}
                        </div>
                    </div>
                </div>
